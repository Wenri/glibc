/* ld.so needs __fstatat64 but must NOT get the android wrapper
   (nix-on-droid/docs/ANDROID-GLIBC.md precondition 4).  Referenced by reached from __stat64/__lstat64; added preemptively.

   Safe to add even if rtld turns out not to need it: this family is RENAMED,
   so fstatat64.os defines none of these names and cannot collide with this copy --
