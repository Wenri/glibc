/* chroot -- Linux implementation, written so the android port can rename it.
...
#if !IS_IN (rtld)
# define chroot __android_next_chroot
...
# define __NR___android_next_chroot __NR_chroot
#endif

int
chroot (const char *path)
{
  return INLINE_SYSCALL_CALL (chroot, path);
}
