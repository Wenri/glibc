/* ld.so needs __lstat64 but must NOT get the android wrapper
   (nix-on-droid/docs/ANDROID-GLIBC.md precondition 4).  Referenced by elf/dl-android-paths.h:218.

   Safe to add even if rtld turns out not to need it: this family is RENAMED,
   so lstat64.os defines none of these names and cannot collide with this copy --
