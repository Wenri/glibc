...
wrapper(lstat64, int, (const char * file_name, struct stat64 * buf))
{
    ...
    /* ADAPTED FOR GLIBC -- not upstream fakechroot.  The `retval == 0' guard and
       the scoping of TMP, matching __lxstat.c:51 and __lxstat64.c:67.  Upstream
       has this guard in three of the four siblings and not here, so BUF -- the
       caller's uninitialised struct stat64 whenever the lstat failed -- decides
       the branch from stack garbage.  */
