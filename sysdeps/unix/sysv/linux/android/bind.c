...
wrapper(bind, int, (int sockfd, BIND_TYPE_ARG2(addr), socklen_t addrlen))
{
    ...
    if (addr_un->sun_family == AF_UNIX && addr_un->sun_path && *(addr_un->sun_path)) {
        ...
        if (af_unix_path != NULL) {
            ...
            /* ADAPTED FOR GLIBC: expand_chroot_path can now fail; strlen()
               below would dereference NULL.  errno is already ENAMETOOLONG,
               which is what the very next test would have set anyway.  */
            if (path == NULL) {
                return -1;
            }
        }
