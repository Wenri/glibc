/* linkat -- Linux implementation, written so the android port can rename it.
...
#if !IS_IN (rtld)
# define linkat __android_next_linkat
...
# define __NR___android_next_linkat __NR_linkat
#endif

int
linkat (int fromfd, const char *from, int tofd, const char *to, int flags)
{
  return INLINE_SYSCALL_CALL (linkat, fromfd, from, tofd, to, flags);
}
