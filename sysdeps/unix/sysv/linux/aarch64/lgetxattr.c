/* lgetxattr -- Linux implementation, written so the android port can rename it.
...
#if !IS_IN (rtld)
# define lgetxattr __android_next_lgetxattr
...
# define __NR___android_next_lgetxattr __NR_lgetxattr
#endif

ssize_t
lgetxattr (const char *path, const char *name, void *value, size_t size)
{
  return INLINE_SYSCALL_CALL (lgetxattr, path, name, value, size);
}
