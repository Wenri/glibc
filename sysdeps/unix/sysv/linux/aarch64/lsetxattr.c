/* lsetxattr -- Linux implementation, written so the android port can rename it.
...
#if !IS_IN (rtld)
# define lsetxattr __android_next_lsetxattr
...
# define __NR___android_next_lsetxattr __NR_lsetxattr
#endif

int
lsetxattr (const char *path, const char *name, const void *value, size_t size, int flags)
{
  return INLINE_SYSCALL_CALL (lsetxattr, path, name, value, size, flags);
}
