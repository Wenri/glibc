/* unlinkat -- Linux implementation, written so the android port can rename it.
...
#if !IS_IN (rtld)
# define unlinkat __android_next_unlinkat
...
# define __NR___android_next_unlinkat __NR_unlinkat
#endif

int
unlinkat (int fd, const char *name, int flag)
{
  return INLINE_SYSCALL_CALL (unlinkat, fd, name, flag);
}
