/* getxattr -- Linux implementation, written so the android port can rename it.
...
#if !IS_IN (rtld)
# define getxattr __android_next_getxattr
...
# define __NR___android_next_getxattr __NR_getxattr
#endif

ssize_t
getxattr (const char *path, const char *name, void *value, size_t size)
{
  return INLINE_SYSCALL_CALL (getxattr, path, name, value, size);
}
