/* removexattr -- Linux implementation, written so the android port can rename it.
...
#if !IS_IN (rtld)
# define removexattr __android_next_removexattr
...
# define __NR___android_next_removexattr __NR_removexattr
#endif

int
removexattr (const char *path, const char *name)
{
  return INLINE_SYSCALL_CALL (removexattr, path, name);
}
