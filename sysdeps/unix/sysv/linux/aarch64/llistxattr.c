/* llistxattr -- Linux implementation, written so the android port can rename it.
...
#if !IS_IN (rtld)
# define llistxattr __android_next_llistxattr
...
# define __NR___android_next_llistxattr __NR_llistxattr
#endif

ssize_t
llistxattr (const char *path, char *list, size_t size)
{
  return INLINE_SYSCALL_CALL (llistxattr, path, list, size);
}
