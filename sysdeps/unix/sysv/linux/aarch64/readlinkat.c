/* readlinkat -- Linux implementation, written so the android port can rename it.
...
#if !IS_IN (rtld)
# define readlinkat __android_next_readlinkat
...
# define __NR___android_next_readlinkat __NR_readlinkat
#endif

ssize_t
readlinkat (int fd, const char *__restrict path, char *__restrict buf, size_t len)
{
  return INLINE_SYSCALL_CALL (readlinkat, fd, path, buf, len);
}
#if IS_IN (rtld)
