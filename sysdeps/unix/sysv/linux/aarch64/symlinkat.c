/* symlinkat -- Linux implementation, written so the android port can rename it.
...
#if !IS_IN (rtld)
# define symlinkat __android_next_symlinkat
...
# define __NR___android_next_symlinkat __NR_symlinkat
#endif

int
symlinkat (const char *from, int tofd, const char *to)
{
  return INLINE_SYSCALL_CALL (symlinkat, from, tofd, to);
}
