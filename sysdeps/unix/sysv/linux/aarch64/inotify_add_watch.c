/* inotify_add_watch -- Linux implementation, written so the android port can rename it.
...
#if !IS_IN (rtld)
# define inotify_add_watch __android_next_inotify_add_watch
...
# define __NR___android_next_inotify_add_watch __NR_inotify_add_watch
#endif

int
inotify_add_watch (int fd, const char *name, uint32_t mask)
{
  return INLINE_SYSCALL_CALL (inotify_add_watch, fd, name, mask);
}
