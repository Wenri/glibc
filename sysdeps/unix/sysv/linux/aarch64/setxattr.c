/* setxattr -- Linux implementation, written so the android port can rename it.
...
#if !IS_IN (rtld)
# define setxattr __android_next_setxattr
...
# define __NR___android_next_setxattr __NR_setxattr
#endif

int
setxattr (const char *path, const char *name, const void *value, size_t size, int flags)
{
  return INLINE_SYSCALL_CALL (setxattr, path, name, value, size, flags);
}
